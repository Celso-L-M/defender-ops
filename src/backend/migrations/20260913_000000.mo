...
/// Migration: AddSecureVault
/// OldActor: exactly matches NewActor from 20260528_600000_AddPerProviderAccessControl.mo (34 fields).
/// NewActor: adds 1 new stable field:
///   - vaultState : { var entries : Map.Map<(ProviderType, Text), VaultEntry>; var key : ?Blob }
///     The centralized per-provider secure vault. `entries` maps each
///     (provider, secret-name) pair to its encrypted entry; `key` holds the
///     SHA-256 encryption key seeded from raw_rand at init (null until the
///     first vault write seeds it). Initialised empty.
module {
  ...
  // ── Vault types (new in this migration) ──────────────────────────────────────
  ...
  type VaultState = {
    var entries : Map.Map<(ProviderType, Text), VaultEntry>;
    var key     : ?Blob;
  };
  ...
  type OldActor = {
    ...
    var providerAssignments          : Map.Map<Principal, [ProviderType]>;
  };

  /// NewActor: adds 1 new stable field for the centralized secure vault.
  type NewActor = {
    var awsCredentials               : ?AwsCredentials;
    var azureCredentials             : ?AzureCredentials;
    var gcpCredentials               : ?GcpCredentials;
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      var enrichmentApiKeys            = old.enrichmentApiKeys;
      var maliciousIpFeed              = old.maliciousIpFeed;
      var lastMaliciousIpFeedPoll      = old.lastMaliciousIpFeedPoll;
      var generatedReports             = old.generatedReports;
      var reportEmailConfigs           = old.reportEmailConfigs;
      var webhookSecrets               = old.webhookSecrets;
      var providerAssignments          = old.providerAssignments;
      var vaultState = {
