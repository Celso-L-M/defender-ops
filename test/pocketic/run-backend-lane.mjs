...
export const SKIP_REASONS = [
  ...
  "no_ic_principal_package",
  "ic_principal_package_unreachable",
...
export function resolveLaneModules(laneRequirePath, projectRoot) {
  ...
  // The lane's test file imports `@dfinity/principal` (for caller isolation) as
  // a bare specifier, which Vite resolves from the importer upward — the same
  // rule that governs `@dfinity/pic`. The principal package is a frontend
  // dependency here, so it lives in `src/frontend/node_modules` and is invisible
  // to a lane file under `app/test/`. A lane that cannot resolve its own imports
  // would boot a replica and then fail the gate on an unresolved module, which
  // is worse than declining. It gets its own reason so the build log names the
  // missing package.
  if (resolveFrom([laneRequirePath], "@dfinity/principal/package.json") === undefined) {
    return {
      skip:
        resolveFrom([appRoot, frontendRoot], "@dfinity/principal/package.json") === undefined
          ? "no_ic_principal_package"
          : "ic_principal_package_unreachable",
    };
  }
